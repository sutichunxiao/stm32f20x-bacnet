...
#define MSTP_TRANSMIT_PACKET_COUNT 2
#endif
    ...
    switch (state) {
        case MSTP_TX_STATE_IDLE:
            if (!Ringbuf_Empty(&Transmit_Queue)) {
				...
				uart_bytes_send(pkt->buffer,pkt->length);
...
extern unsigned char rxBuff[256];
        ...
        case MSTP_RECEIVE_STATE_IDLE:
            ...
            if (flag_uart == 1) {
                ...
                if (rxBuff[0] == 0x55 ) {
					...
					else if(rxBuff[1] == 0x55 &&rxBuff[2] == 0x55)
					{
						eq = 1;
					}
					...
					for(i = 0; i <= 5; i++)
					{
						HeaderCRC = CRC_Calc_Header(rxBuff[i+2+eq], HeaderCRC);
					}
					DataLength = rxBuff[5+eq]*256 + rxBuff[6+eq];
					FrameType = rxBuff[2+eq];
					DestinationAddress = rxBuff[3+eq];
					SourceAddress = rxBuff[4+eq];
					if (HeaderCRC != 0x55) {
                        /* BadCRC */
                        ...
                        if (DataLength == 0) {
							...
							{
								DataCRC = CRC_Calc_Data(rxBuff[i+8+eq], DataCRC);
								if (Index < InputBufferSize) 
								{
									InputBuffer[i] = rxBuff[i+8+eq];
								}
							}
							DataCRC = CRC_Calc_Data(rxBuff[i+8+eq], DataCRC);
							DataCRC = CRC_Calc_Data(rxBuff[i+9+eq], DataCRC);
							if (DataCRC == 0xF0B8) 
							{
								if ((DestinationAddress == This_Station) ||(DestinationAddress ==
                                    MSTP_BROADCAST_ADDRESS)) 
								{
									/* ForUs */
									MSTP_Flag.ReceivedValidFrame = true;
